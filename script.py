...
def checkPlatform():
    ...
    logging.info("OS: %s %s, aka \"%s\""%(d["_system"], d["_release"], d["_os"]))
...
def checkPython():
    ...
    try:
        ...
        logging.info("Python version %s.%s.%s"%(sys.version_info.major, sys.version_info.minor, sys.version_info.micro))
...
def main():
    ...
    remember(_timestamp="%s"%(datetime.datetime.now().strftime("%d-%m-%Y %H:%M:%S")))    
    try:
